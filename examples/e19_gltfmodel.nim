...
#:____________________________________________________
# glTF Model Example                        |
# Draws a multi-mesh glTF using Tech.Simple |
...
"""
const Spheres   = Path currentSourcePath().parentDir()/"mdl"/"mrSpheres"/"MetalRoughSpheres.gltf"
const IsoCube   = Path currentSourcePath().parentDir()/"mdl"/"cube"/"isocube.gltf"
const ModelFile = Spheres
...
proc run=
  ...
  var model   = e.render.new(RenderModel, mdl.load(ModelFile))         # Create the glTF RenderModel
  var tech    = e.render.init(Tech.Simple,
    code = shaderCode,
    data = (uniform,texture).mvar, # The simple tech only accepts a tuple of Group.global data
    ) # << Tech.Simple.init( ... )
  e.render.upload(model)  # Upload the mesh to the GPU
  ...
  while not e.sys.close():
    ...
    # Render this model, with this style
    e.render.draw(model, tech)  # (note: uses any global data contained in the style)
  #__________________
  # Terminate
  e.render.term()
  e.sys.term()
